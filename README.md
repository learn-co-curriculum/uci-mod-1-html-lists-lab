...
- Identify unordered and ordered HTML lists
...
Let's say we are building a personal website and want to list
out our favorite foods. We _can_ write this like so:
...
This would create a new line on the page for each food but doesn't really
...
The first part of this challenge is to make the first two tests pass by:
...
of bullets. If we wanted to write a _ranked_ list of favorite foods, it
might look like:
...
Lists in HTML are very useful for organizing related content and are the only
way to indicate that content is related using basic HTML. Unordered lists are
useful for more than just listing favorite foods and ingredients. It's possible,
for instance, to use list elements to organize navigation links - with styling,
we make the content look however we want: remove the bullets, make them line up
horizontally, but in our HTML they will still be organized and easy to read.
